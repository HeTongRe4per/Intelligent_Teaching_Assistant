<template>
  <div class="content-wrapper">
    <div class="query-wrapper">
      <el-form ref="queryFormRef" :model="queryParams">
        <el-row :gutter="15">
          ...
          <el-col :xs="8" :sm="8" :md="8" :lg="6" :xl="6">
            <el-form-item label="权限" name="permissionId" prop="permissionId">
              <var-field-input
                v-model="queryParams.permissionId"
                :recordMeta="{ name: 'permissionId', title: '权限', dataType: 'Integer', selectable: true, options: permissionIdOptions }"/>
            </el-form-item>
          </el-col>
      ...
      <div class="table-body-wrapper">
        <el-table :data="tableData" :row-key="(row: any) => row.id" @selection-change="tableRowSelection">
          <template #default>
            ...
            <el-table-column prop="permissionId" label="权限编号" align="center">
              <template #default="scope">
                {{ permissionIdOptions.find((item: any) => item.value === scope.row.permissionId).label }}
              </template>
            </el-table-column>
    ...
    <modal-form ref="modalFormRef" :hide-id-field="true" title="角色权限关联表信息" @handleAdd="handleAdd" @handleEdit="handleEdit" />
  </div>
</template>
...
const permissionIdOptions = ref<any>([]);
const getPermissionIdOptions = () => {
  request.get("/manage/permission/list", {
    pageSize: 1000
  }).then((data: any) => {
    permissionIdOptions.value = data.records.map((item: any) => {
      return {
        label: item.name,
        value: item.id
      };
...
const recordMetas = [
  ...
  },
  {
    ...
    dataType: "Integer",
    selectable: true,
    options: permissionIdOptions
  },
];
const urls = {
  list: "/manage/rolePermission/list",
  detail: "/manage/rolePermission/queryById",
  add: "/manage/rolePermission/add",
  edit: "/manage/rolePermission/edit",
  delete: "/manage/rolePermission/deleteById",
  batchDelete: "/manage/rolePermission/batchDeleteById"
};
let { tableData, pagination, handleList, handleAdd, handleEdit, handleDelete, handleBatchDelete } =
  useCrudActions(urls, queryParams);
onMounted(() => {
  getRoleIdOptions();
  getPermissionIdOptions();
  setTimeout(() => {
    handleList();
  }, 500);
});
